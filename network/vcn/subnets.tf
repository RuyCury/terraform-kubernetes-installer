...
resource "oci_core_subnet" "PublicSubnetAD1" {
  ...
  security_list_ids   = "${concat(list(oci_core_security_list.PublicSecurityList[count.index].id), var.additional_public_security_lists_ids)}"
...
resource "oci_core_subnet" "NATSubnetAD1" {
  ...
  vcn_id              = "${oci_core_virtual_network.CompleteVCN.id}"
  route_table_id      = "${oci_core_route_table.PublicRouteTable.id}"
  security_list_ids   = "${concat(list(oci_core_security_list.NatSecurityList[count.index].id), var.additional_nat_security_lists_ids)}"
  dhcp_options_id     = "${oci_core_virtual_network.CompleteVCN.default_dhcp_options_id}"
}
